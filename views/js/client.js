...
socket.on('login', ({ profileImage }) => {
  ...
  // Reemplazar la imagen de perfil predeterminada con la imagen del usuario
  const userImageElement = document.querySelector(".userimg img");
  userImageElement.src = profileImage || "img/user.png"; // Si no se proporciona una imagen, utiliza la predeterminada
});
...
socket.on('userMessages', (messages) => {
  messages.forEach(({ message, user, image }) => {
    printMessages.insertAdjacentHTML(
      "beforeend",
      `<div class="message frnd_message"><p>${message}<br /><span>${user}</span></p></div>`
    );
  ...
  if (txtUserNickName.value.trim() !== "") {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        socket.emit("register", { username, fileDataURL });
      };
      reader.readAsDataURL(file);
    } else {
      socket.emit("register", { username, fileDataURL: null });
    }
  }
});
